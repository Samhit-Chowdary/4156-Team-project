...
public class PayrollServiceTests {
    ...
    @BeforeEach
    void setUp() {
        payrollService = new PayrollService(payrollRepository, employeeProfileService, companyEmployeesService, pdfGenerator);
    ...
    void testGetPayrollByEmployeeIdEmployeeNotFound() {
        ...
        assertEquals(Pair.of(PayrollStatus.EMPLOYEE_NOT_FOUND, new ArrayList<>()), payrollService.getPayrollByEmployeeId(1));
    }
    ...
    void testGetPayrollByEmployeeIdSuccess() {
        ...
        when(payrollRepository.findAllByEmployeeIdOrderByPaymentDateDesc(1)).thenReturn(List.of(payroll));
        assertEquals(Pair.of(PayrollStatus.OK, List.of(payrollInfo)), payrollService.getPayrollByEmployeeId(1));
    }
    ...
    void testAdjustSalaryByEmployeeIdEmployeeNotFound() {
        Map<String, Object> updates = Map.of("month", 10, "year", 2024);
        when(companyEmployeesService.verifyIfEmployeeInCompany(1)).thenReturn(false);
        assertEquals(PayrollStatus.EMPLOYEE_NOT_FOUND, payrollService.adjustSalaryByEmployeeId(1, updates));
    }
    ...
    void testAdjustPaymentDayByEmployeeIdEmployeeNotFound() {
        ...
        when(companyEmployeesService.verifyIfEmployeeInCompany(1)).thenReturn(false);
        assertEquals(PayrollStatus.EMPLOYEE_NOT_FOUND, payrollService.adjustPaymentDayByEmployeeId(1, updates));
    }
    ...
    void testAdjustPaymentDayByEmployeeIdInvalidData() {
        Map<String, Object> updates = new HashMap<>();
        when(companyEmployeesService.verifyIfEmployeeInCompany(1)).thenReturn(true);
        assertEquals(PayrollStatus.INVALID_DATA, payrollService.adjustPaymentDayByEmployeeId(1, updates));
    }
    ...
    void testAdjustPaymentDayByEmployeeIdInvalidFormat() {
        ...
        when(companyEmployeesService.verifyIfEmployeeInCompany(1)).thenReturn(true);
        assertEquals(PayrollStatus.INVALID_FORMAT, payrollService.adjustPaymentDayByEmployeeId(1, updates));
    }
    ...
    void testAddPayrollByEmployeeIdEmployeeNotFound() {
        Map<String, Object> updates = new HashMap<>();
        when(companyEmployeesService.verifyIfEmployeeInCompany(1)).thenReturn(false);
        assertEquals(PayrollStatus.EMPLOYEE_NOT_FOUND, payrollService.addPayrollByEmployeeId(1, updates));
    }
    ...
    void testDeletePayrollByEmployeeIdEmployeeNotFound() {
        Map<String, Object> updates = new HashMap<>();
        when(companyEmployeesService.verifyIfEmployeeInCompany(1)).thenReturn(false);
        assertEquals(PayrollStatus.EMPLOYEE_NOT_FOUND, payrollService.deletePayrollByEmployeeId(1, updates));
    }
    ...
    void testDeletePayrollByEmployeeIdSuccess() {
        ...
        when(companyEmployeesService.verifyIfEmployeeInCompany(1)).thenReturn(true);
        when(payrollRepository.findByEmployeeIdPaymentMonthAndYear(1, 10, 2024)).thenReturn(new Payroll());
    ...
    void testGeneratePayrollInvalidData() {
        ...
        assertEquals(Map.of("response", "Invalid day or month or year"), payrollService.generatePayroll(updates));
    }

    @Test
    void testGeneratePayrollInvalidFormat() {
        Map<String, Object> updates = new HashMap<>(Map.of("day", 32, "month", 10, "year", 2024));
        EmployeeProfile employeeProfile1 = new EmployeeProfile();
        employeeProfile1.setId(100);
        ...
        when(employeeProfileService.getEmployeeProfile(200)).thenReturn(Optional.of(employeeProfile2));
        when(companyEmployeesService.verifyIfEmployeeInCompany(200)).thenReturn(true);
        assertEquals(Map.of("response", "Invalid format for day or month or year"), payrollService.generatePayroll(updates));
    }
    ...
    void testGeneratePayrollAlreadyExists() {
        ...
        result.put("response", "Payroll for the employees in list have already been generated and were not added");
    ...
    void testGeneratePayrollSuccess() {
        ...
        when(payrollRepository.findByEmployeeIdPaymentMonthAndYear(200, 10, 2024))
                .thenReturn(null);
        assertEquals(Map.of("response", "Payroll for this month and year has been generated"), payrollService.generatePayroll(updates));
    }
    ...
    void testDeletePayrollInvalidData() {
        ...
        when(employeeProfileService.getEmployeeProfile(200)).thenReturn(Optional.of(employeeProfile2));
        when(companyEmployeesService.verifyIfEmployeeInCompany(200)).thenReturn(true);
        assertEquals(Map.of("response", "Invalid month or year"), payrollService.deletePayroll(updates));
    }

    @Test
    void testDeletePayrollInvalidFormat() {
        Map<String, Object> updates = new HashMap<>(Map.of("day", 32, "month", 13, "year", 2024));
        EmployeeProfile employeeProfile1 = new EmployeeProfile();
        employeeProfile1.setId(100);
        EmployeeProfile employeeProfile2 = new EmployeeProfile();
        employeeProfile2.setId(200);
        ...
        when(employeeProfileService.getEmployeeProfile(200)).thenReturn(Optional.of(employeeProfile2));
        when(companyEmployeesService.verifyIfEmployeeInCompany(200)).thenReturn(true);
        assertEquals(Map.of("response", "Invalid format for month or year"), payrollService.deletePayroll(updates));
    }
    ...
    void testDeletePayrollSuccess() {
        ...
        when(payrollRepository.findByEmployeeIdPaymentMonthAndYear(200, 10, 2024))
                .thenReturn(null);
        assertEquals(Map.of("response", "Payroll for this month and year has been deleted"), payrollService.deletePayroll(updates));
    }
}
