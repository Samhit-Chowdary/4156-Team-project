...
public class CompanyControllerTests {
    ...
    void testRegisterCompanyUsernameExists() {
        when(companyService.registerCompany(any(Company.class))).thenReturn("company with same username already exists");
        Company company = new Company();
        company.setPassword("password");
        company.setUsername("username");
        ResponseEntity<?> response = companyController.registerCompany(company);
        assertEquals(Map.of("response", "company with same username already exists"), response.getBody());
        assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
    }
