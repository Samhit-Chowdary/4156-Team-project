...
public interface TimeOffRepository extends JpaRepository<TimeOff, Integer> {
  ...
  @Query(
      value =
          "SELECT * FROM timeoff t WHERE t.employee_id = ?1 AND t.start_date >= ?2 "
                  + "AND t.end_date <= ?3 ORDER BY t.start_date ASC",
      nativeQuery = true)
  List<TimeOff> findAllByEmployeeIdGivenDateRange(
